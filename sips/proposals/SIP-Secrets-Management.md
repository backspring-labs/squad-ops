...
- [ ] `secret://` references are supported across config surfaces
- [ ] `secrets.name_map` is supported and validated
- [ ] Providers `env`, `file`, `docker_secret` implemented
- [ ] No plaintext secrets/default creds in repo
- [ ] CI fails on forbidden patterns
- [ ] DB/MQ/auth bootstraps use Secrets Manager
