...
**Amended 2026-09-12 (owner's ruling, `docs/plans/1-8-0-plan.md` §8):** co-headliner with **Scoped Code Revision** (Lane M), not Campaign, which is v2.0's headline. The 1.8 slice is **headless and mechanical** — `CycleAssessment` over `CycleOutcome`, one failure-attribution registry shared with `FailureEvidenceCategory`, `FailureLocus`, the plan-validation rejection classes, the winnability classes and the correction movement tokens, a benchmark registry over the stored counted rolls, and the comparison harness with one pre-registered window; the console page moves to SIP-0069's surface as a later consumer, and no agent sits in the assessment path. **Rev 2 of this SIP is written to that slice before its design review** (the 1.8 plan §3.1). The paragraph below is the superseded 2026-08-07 billing, kept as the record.
