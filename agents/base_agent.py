...
class BaseAgent(ABC):
    ...
    def __init__(self, name: str, agent_type: str, reasoning_style: str):
        ...
        # TaskLogger will be initialized per-cycle, so we store the class reference
        self._TaskLogger = TaskLogger
    ...
    def create_span(
        self, span_name: str, attributes: dict[str, Any] = None, kind: str | None = None
    ):
    ...
    def _load_agent_info(self) -> dict[str, Any] | None:
        ...
        if docker_path.exists():
            try:
                ...
                logger.warning(
                    f"{self.name}: Failed to load agent_info.json from {docker_path}: {e}"
                )
        ...
        if local_path.exists():
            try:
                ...
                logger.warning(
                    f"{self.name}: Failed to load agent_info.json from {local_path}: {e}"
                )

        # File not found - backward compatibility
        logger.warning(
            f"{self.name}: agent_info.json not found, continuing without metadata (backward compatibility)"
        )
        return None
    ...
    async def _announce_agent_online(self, agent_info: dict[str, Any]):
        ...
        try:
            ...
            await self.broadcast_message(
                message_type="agent_online", payload=payload, context={"source": "agent_startup"}
            )

            logger.info(
                f"{self.name}: Announced agent_online with build_hash={agent_info.get('build_hash')}"
            )
        except Exception as e:
    ...
    async def initialize(self):
        ...
        try:
            ...
            if agent_info:
                ...
                logger.info("agent_runtime_identity", extra={"agent_info": agent_info})
    ...
    async def _store_role_context(self):
        ...
        try:
            ...
            if role_definition:
                ...
                reasoning_explanation = self._get_reasoning_explanation(
                    role_definition.reasoning_style
                )
...
"""
                ...
                role_context = (
                    f"You are {self.name}, a {self.agent_type} agent in the SquadOps system.\n\n"
                )
            ...
            if self.memory_provider:
                try:
                    await self.record_memory(
                        ...
                        payload={
                            ...
                            if role_definition
                            else f"{self.agent_type} agent",
                            "reasoning_style": role_definition.reasoning_style
                            if role_definition
                            else self.reasoning_style,
                ...
                logger.warning(
                    f"{self.name}: Memory provider not available, skipping role context storage"
                )
    ...
    def task_logger(self):
        """
        Get TaskLogger instance for current cycle (SIP-0048).

        Returns:
            TaskLogger instance configured with agent_id and current cycle_id
        """
        from agents.utils.task_logger import TaskLogger

        # Get current cycle_id from context or use a default
        cycle_id = getattr(self, "current_cycle_id", None) or "unknown"
        return TaskLogger(
            runtime_api_url=self.runtime_api_url,
            agent_id=self.name,  # Use agent name as agent_id
            cycle_id=cycle_id,
        )
    ...
    async def record_memory(
        ...
        if ns == "squad":
            if not self.sql_adapter:
                logger.debug(
                    f"{self.name}: SQL adapter not initialized, skipping squad memory recording"
                )
                return None
            ...
            if not self.memory_provider:
                logger.debug(
                    f"{self.name}: Memory provider not initialized, skipping memory recording"
                )
                return None
            adapter = self.memory_provider

        try:
            # Extract PID and cycle_id from context (SIP-0048: renamed from ECID)
            ...
            cycle_id = context_info.get("cycle_id", "unknown")  # SIP-0048: renamed from ecid
            ...
            # Create tags
            tags = [f"pid:{pid}", kind, f"agent:{self.name.lower()}"]
            if cycle_id != "unknown":  # SIP-0048: renamed from ecid
                tags.append(f"cycle_id:{cycle_id}")  # SIP-0048: renamed from ecid
            ...
            memory_content = {
                ...
                "cycle_id": cycle_id,  # SIP-0048: renamed from ecid
                "pid": pid,
            }
            ...
            memory_item = {
                ...
                "cycle_id": cycle_id
                if cycle_id != "unknown"
                else None,  # SIP-0048: renamed from ecid
            }
            ...
            with self.create_span(span_name, span_attrs):
                ...
                self.record_histogram(
                    ...
                    latency_ms,
                    {
                        "operation": "put_if_not_exists"
                        if (
                            kind == "role_identity"
                            and ns == "role"
                            and adapter == self.memory_provider
                        )
                        else "put",
                        "namespace": ns,
                        "agent": self.name,
                ...
                if mem_id:
                    ...
                    logger.info(
                        f"{self.name}: Recorded memory {kind} (ID: {mem_id[:8] if mem_id else 'None'}...) for cycle_id {cycle_id} in {storage} ({latency_ms:.2f}ms)"
                    )  # SIP-0048: renamed from ECID/ecid
                else:
                    ...
                    logger.warning(
                        f"{self.name}: Failed to record memory {kind} - adapter returned None"
                    )
    ...
    async def retrieve_memories(
        self, query: str = "", k: int = 8, ns: str = "role", **kwargs
    ) -> list[dict]:
        """
        ...
        Args:
            ...
            **kwargs: Additional filters (tags, agent, pid, cycle_id, etc.)  # SIP-0048: renamed from ecid
        ...
        if ns == "squad":
            if not self.sql_adapter:
                logger.debug(
                    f"{self.name}: SQL adapter not initialized, cannot retrieve squad memories"
                )
                return []
            adapter = self.sql_adapter
        else:
            if not self.memory_provider:
                logger.debug(
                    f"{self.name}: Memory provider not initialized, cannot retrieve memories"
                )
                return []
            adapter = self.memory_provider
        ...
        try:
            with self.create_span(span_name, span_attrs):
                ...
                self.record_histogram(
                    "memory_operation_latency_ms",
                    latency_ms,
                    {"operation": "get", "namespace": ns, "agent": self.name},
                )
                ...
                self.record_counter(
                    "memory_operations_total",
                    1,
                    {"operation": "get", "namespace": ns, "agent": self.name, "status": "success"},
                )

                # Record results count as gauge
                self.record_gauge(
                    "memory_retrieval_results_count",
                    len(memories),
                    {"namespace": ns, "agent": self.name},
                )

                logger.debug(
                    f"{self.name}: Retrieved {len(memories)} memories from {ns} namespace ({latency_ms:.2f}ms)"
                )
                return memories
            ...
            self.record_counter(
                "memory_operations_total",
                1,
                {"operation": "get", "namespace": ns, "agent": self.name, "status": "error"},
            )
    ...
    async def broadcast_message(
        self, message_type: str, payload: dict[str, Any], context: dict[str, Any] = None
    ):
    ...
    async def update_task_status(
        self, task_id: str, status: str, progress: float = 0.0, eta: str = None
    ):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with (
                    session.post(
                        f"{self.runtime_api_url}/api/v1/task-status",  # SIP-0048: renamed from task_api_url
                        json={
                            "task_id": task_id,
                            "agent_name": self.name,
                            "status": status,
    ...
    async def create_execution_cycle(
        ...
        cycle_id: str,
        pid: str,
        run_type: str,  # SIP-0048: renamed from ecid
        title: str,
        description: str = None,
        project_id: str = None,
    ):
        """Create execution cycle via API (SIP-0047: supports project_id, SIP-0048: uses cycle_id)"""
        async with aiohttp.ClientSession() as session:
            payload = {
                "cycle_id": cycle_id,  # SIP-0048: renamed from ecid
                "pid": pid,
            ...
            async with (
                session.post(
                    f"{self.runtime_api_url}/api/v1/execution-cycles",  # SIP-0048: renamed from task_api_url
                    json=payload,
                ) as resp
            ):
                if resp.status != 200:
    ...
    async def log_task_start(
        ...
        task_id: str,
        cycle_id: str,
        description: str,  # SIP-0048: renamed from ecid
        priority: str = "MEDIUM",
        dependencies: list[str] = None,
        delegated_by: str = None,
        delegated_to: str = None,
    ):
        """Log task start via API (SIP-0048: uses cycle_id)"""
        try:
            async with aiohttp.ClientSession() as session:
                async with (
                    session.post(
                        f"{self.runtime_api_url}/api/v1/tasks/start",  # SIP-0048: renamed from task_api_url
                        json={
                            "task_id": task_id,
                            "cycle_id": cycle_id,  # SIP-0048: renamed from ecid
                            "agent": self.name,
    ...
    async def log_task_delegation(
        ...
        task_id: str,
        cycle_id: str,
        delegated_to: str,  # SIP-0048: renamed from ecid
        description: str,
    ):
    ...
    async def log_task_completion(self, task_id: str, artifacts: dict[str, Any] = None):
        ...
        async with aiohttp.ClientSession() as session:
            async with (
                session.post(
                    f"{self.runtime_api_url}/api/v1/tasks/complete",  # SIP-0048: renamed from task_api_url
                    json={"task_id": task_id, "artifacts": artifacts},
                ) as resp
            ):
                if resp.status != 200:
    ...
    async def log_task_failure(self, task_id: str, error_log: str):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                f"{self.runtime_api_url}/api/v1/tasks/fail",  # SIP-0048: renamed from task_api_url
                json={"task_id": task_id, "error_log": error_log},
            ) as resp:
                if resp.status != 200:
                    logger.error(f"Failed to log task failure: {await resp.text()}")
                return await resp.json()

    async def update_execution_cycle_status(
        self, cycle_id: str, status: str, notes: str = None
    ):  # SIP-0048: renamed from ecid
        """Update execution cycle status via API (SIP-0048: uses cycle_id)"""
        async with aiohttp.ClientSession() as session:
            async with (
                session.put(
                    f"{self.runtime_api_url}/api/v1/execution-cycles/{cycle_id}",  # SIP-0048: renamed from task_api_url and ecid
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if resp.status != 200:
                        ...
                        logger.debug(
                            f"{self.name} heartbeat sent via Health-Check API (memory_count: {memory_count})"
                        )
                        return await resp.json()
        except Exception as e:
    ...
    def _load_capability_config(self):
        ...
        try:
            ...
            self.implemented_capabilities = (
                loader.get_agent_capabilities(role) if self.capability_config else []
            )

            logger.info(
                f"{self.name}: Loaded capability config for role {role}, implements {len(self.implemented_capabilities)} capabilities"
            )
    ...
    def _validate_constraints(self, request: "AgentRequest") -> tuple[bool, str | None]:
        ...
        if "repo_allow" in constraints:
            ...
            if (
                payload_repo
                and repo_allow
                and not any(repo_allow_pattern in payload_repo for repo_allow_pattern in repo_allow)
            ):
    ...
    async def handle_reasoning_event(self, message: AgentMessage) -> None:
        ...
        try:
            ...
            reasoning_data = {
                ...
                or context.get("cycle_id", "unknown"),  # SIP-0048: renamed from ecid
                "task_id": payload.get("task_id", "unknown"),
            ...
            cycle_id = payload.get("cycle_id", "unknown")  # SIP-0048: renamed from ecid
            logger.info(
                f"{self.name} received reasoning event from {sender}: {payload.get('reason_step', 'unknown')} for {payload.get('context', 'unknown')} (cycle_id: {cycle_id})"
            )  # SIP-0048: renamed from ECID
    ...
    async def llm_response(self, prompt: str, context: str = "") -> str:
        ...
        span_ctx = self.create_span(
            span_name,
            {
                ...
                "cycle_id": getattr(self, "current_cycle_id", None),  # SIP-0048: renamed from ecid
            },
        )
                    ...
                    if hasattr(self.llm_client, "get_token_usage"):
                        ...
                        if token_usage:
                            ...
                            if cycle_id:  # SIP-0048: renamed from ecid
                                labels["cycle_id"] = cycle_id  # SIP-0048: renamed from ecid

                            self.record_counter("agent_tokens_used_total", total_tokens, labels)
                            logger.debug(
                                f"{self.name} LLM call used {total_tokens} tokens ({token_usage.get('prompt_tokens', 0)} prompt + {token_usage.get('completion_tokens', 0)} completion)"
                            )
                except Exception as e:
    ...
    async def run(self):
        ...
        try:
                        ...
                        if start_metrics_server:
                            ...
                            self.metrics_server = await start_metrics_server(
                                ...
                                meter_provider=getattr(
                                    self.telemetry_client, "meter_provider", None
                                ),
                                prometheus_reader=prometheus_reader,
                            )
                            logger.info(
                                f"{self.name} started metrics HTTP server on port {prometheus_port}"
                            )
                except Exception as e:
            ...
            task_queue = await self.channel.declare_queue(
                f"{self.name.lower()}_tasks", durable=True
            )
            comms_queue = await self.channel.declare_queue(
                f"{self.name.lower()}_comms", durable=True
            )
                    ...
                    try:
                        ...
                        if "action" in msg_data:
                            ...
                            if response_queue:
                                ...
                                logger.info(
                                    f"{self.name} sent response to {response_queue} for {action}"
                                )
                        else:
            ...
            await asyncio.gather(
                process_tasks(), process_comms(), process_broadcasts(), heartbeat_loop()
            )
