...
class DockerBuilder:
    ...
    async def build(self, task_id: str, requirements: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"{self.name} parsed manifest: {manifest.get('architecture_type', 'unknown')}"
            )
            ...
            build_requirements = {
                ...
                "success_criteria": requirements.get(
            ...
            if not await self.file_manager.directory_exists(app_dir):
                logger.info(
                    f"{self.name} app directory doesn't exist, creating files from manifest"
                )
                ...
                app_name_kebab = (
                    re.sub(
                        r"([a-z0-9])([A-Z])",
                        r"\1-\2",
                        build_requirements.get("app_name", "application"),
                    )
                    .lower()
                    .replace(" ", "-")
                )
                ...
                files = await app_builder.generate_files_json(
                ...
                for file_info in files:
                    if file_info["type"] == "create_file":
                        result = await self.file_manager.create_file(
                            file_info["file_path"], file_info["content"], file_info.get("directory")
                        )
                ...
                logger.info(
                    f"{self.name} app directory exists, files already created by design task"
                )
            ...
            return {"task_id": task_id, "status": "error", "error": str(e), "action": "build"}
