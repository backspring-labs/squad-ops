...
class TelemetryCollector:
    ...
    async def collect(self, ecid: str, task_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'execution_duration' in telemetry['database_metrics']:
                telemetry['execution_duration'] = telemetry['database_metrics']['execution_duration']
                    ...
                    if resp.status == 200:
                        ...
                        try:
                            ...
                            if start_time_str:
                                ...
                                logger.info(f"{self.name} execution duration: {metrics['execution_duration']['duration_formatted']}")
                        except Exception as e:
    ...
    async def collect_rabbitmq_metrics(self, ecid: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await self.agent.execute_command("rabbitmqctl list_queues name messages consumers")
            queue_stats = {}
    ...
    async def collect_system_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                result = await self.agent.execute_command("nvidia-smi --query-gpu=utilization.gpu,memory.used,memory.total --format=csv,noheader,nounits")
                if result.get('success') and result.get('stdout'):
    ...
    async def collect_docker_events(self, database_metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            cycle_start = None
            if 'execution_duration' in database_metrics and database_metrics.get('execution_duration', {}).get('start_time'):
            ...
            result = await self.agent.execute_command(f"docker events --since {since_time} --format '{{{{.Time}}}} {{{{.Type}}}} {{{{.Action}}}} {{{{.Actor.Attributes.name}}}}'")
