...
class LanceDBAdapter(MemoryProvider):
    ...
    def _create_table(self):
        ...
        try:
            ...
            schema = pa.schema([
                ...
                pa.field('vector', pa.list_(pa.float32(), 768))  # Embedding vector (768 for Ollama)
            ])
    ...
    def _generate_embedding(self, text: str) -> List[float]:
        ...
        try:
            ...
            if len(embedding) == 768:
                ...
                logger.warning(f"{self.agent_name}: Ollama returned {len(embedding)} dims, expected 768, using fallback")
                raise ValueError(f"Unexpected embedding dimension: {len(embedding)}")
                
        except Exception as e:
            logger.debug(f"{self.agent_name}: Ollama embedding failed: {e}, trying SentenceTransformers")
            ...
            try:
                ...
                if len(embedding) == 384:
                    ...
                    logger.debug(f"{self.agent_name}: Using SentenceTransformers (384d) padded to 768d")
                else:
                    logger.warning(f"{self.agent_name}: SentenceTransformers returned unexpected dimension: {len(embedding)}")
    ...
    async def put(self, item: dict) -> str:
        ...
        try:
            ...
            logger.info(f"{self.agent_name}: Stored memory {mem_id} in LanceDB table {self._table_name}")
            return mem_id
    ...
    async def get(self, query: str, k: int = 8, **kw) -> List[dict]:
        ...
        """
        if self._table is None:
            logger.warning(f"{self.agent_name}: LanceDB table not initialized, cannot retrieve memories.")
            return []
        ...
        try:
            ...
            logger.debug(f"{self.agent_name}: Embedding generated successfully, dimension: {len(query_embedding)}")
            ...
            if mem_ids_filter and isinstance(mem_ids_filter, (list, tuple)) and len(mem_ids_filter) > 0:
            ...
            for _, row in results_df.iterrows():
                ...
                if isinstance(content, str):
                    try:
                        ...
                        logger.warning(f"{self.agent_name}: Failed to parse content as JSON: {content[:50]}")
                        content = {}
