...
class LeadAgent(BaseAgent):
    ...
    def __init__(self, identity: str, instances_file: str = "agents/instances/instances.yaml"):
        super().__init__(name=identity, agent_type="governance", reasoning_style="governance")
        self.task_state_log = []
        self.approval_queue = []
        ...
        self.warmboot_state = {"manifest": None, "build_files": [], "pending_tasks": []}
    ...
    async def process_task(self, task: dict[str, Any]) -> dict[str, Any]:
        ...
        cycle_id = task.get("cycle_id") or task.get("context", {}).get(
            "cycle_id"
        )  # SIP-0048: renamed from ecid
        pid = task.get("pid") or task.get("context", {}).get("pid")
        if (cycle_id or pid) and self.capability_loader:  # SIP-0048: renamed from ecid
            try:
                await self.capability_loader.execute(
                    "warmboot.memory", self, cycle_id, pid
                )  # SIP-0048: renamed from ecid
            except Exception as e:
        ...
        try:
            ...
            return {"task_id": task_id, "status": "error", "error": f"Capability not found: {e}"}
        except Exception as e:
            logger.error(f"{self.name}: Capability execution error: {e}", exc_info=True)
            await self.update_task_status(task_id, "Failed", 0.0, str(e))
    ...
    async def handle_message(self, message: AgentMessage) -> None:
        ...
        if message.message_type == "approval_request":
            ...
            logger.info(
                f"{self.name} received message: {message.message_type} from {message.sender}"
            )
    ...
    async def handle_prd_request(self, message: AgentMessage) -> None:
        ...
        if "task_id" not in task:
            cycle_id = task.get("cycle_id") or message.context.get(
                "cycle_id", "CYCLE-WB-001"
            )  # SIP-0048: renamed from ecid
            task["task_id"] = (
                f"{cycle_id}-prd-{int(__import__('time').time())}"  # SIP-0048: renamed from ecid
            )

        # Merge context fields into task if needed
        if "cycle_id" not in task:  # SIP-0048: renamed from ecid
            task["cycle_id"] = message.context.get(
                "cycle_id", "CYCLE-WB-001"
            )  # SIP-0048: renamed from ecid
        ...
        await self.send_message(
            ...
            payload=result,
            context={"original_request": message.payload, "processed_by": self.name},
        )
    ...
    async def handle_task_delegation(self, message: AgentMessage) -> None:
        ...
        try:
            ...
            logger.info(
                f"{self.name} received task delegation: {task_id} (type: {task_type}) from {message.sender}"
            )
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.put(
                        ...
                        if resp.status == 200:
                            ...
                            cycle_id = task_payload.get("cycle_id", "unknown")
            ...
            logger.info(
                f"{self.name} completed delegated task {task_id}: {result.get('status', 'unknown')}"
            )
