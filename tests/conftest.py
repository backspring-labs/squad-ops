...
def _detect_integration_tests():
    ...
    for arg in sys.argv:
        ...
        if "integration" in arg and (
            "test" in arg.lower() or arg.endswith(".py") or "/" in arg or "\\" in arg
        ):
            return True
...
def reset_path_resolver():
    ...
    try:
        ...
        # PathResolver not available (e.g., in new DDD structure)
        # This is fine - just skip the reset
        yield
