...
class TestArtifactsPluginRegistration:
    ...
    def test_panel_artifacts_list(self, mock_ctx):
        ...
        list_panels = [p for p in panels if p[0][1]["component"] == "squadops-artifacts-list"]
    ...
    def test_panel_artifacts_browser(self, mock_ctx):
        register(mock_ctx)
        panels = [c for c in mock_ctx.register_contribution.call_args_list if c[0][0] == "panel"]
        browser_panels = [p for p in panels if p[0][1]["component"] == "squadops-artifacts-browser"]
    ...
    def test_panel_artifacts_detail_in_right_rail(self, mock_ctx):
        register(mock_ctx)
        panels = [c for c in mock_ctx.register_contribution.call_args_list if c[0][0] == "panel"]
        detail_panels = [p for p in panels if p[0][1]["component"] == "squadops-artifacts-detail"]
    ...
    def test_panel_artifacts_viewer_in_modal(self, mock_ctx):
        register(mock_ctx)
        panels = [c for c in mock_ctx.register_contribution.call_args_list if c[0][0] == "panel"]
        viewer_panels = [p for p in panels if p[0][1]["component"] == "squadops-artifacts-viewer"]
