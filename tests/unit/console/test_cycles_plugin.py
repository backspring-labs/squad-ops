...
_plugin_path = (
    Path(__file__).parents[3] / "console" / "continuum-plugins" / "squadops.cycles" / "__init__.py"
)
...
class TestCyclesPluginRegistration:
    ...
    def test_total_contributions(self, mock_ctx):
        """1 nav + 1 composite panel + 7 commands = 9 contributions (SIP-0074)."""
        register(mock_ctx)
        assert mock_ctx.register_contribution.call_count == 9
    ...
    def test_command_open_create_cycle_is_safe(self, mock_ctx):
        register(mock_ctx)
        commands = [
            c for c in mock_ctx.register_contribution.call_args_list if c[0][0] == "command"
        ]
        occ = [c for c in commands if c[0][1]["id"] == "squadops.open_create_cycle"]
        assert len(occ) == 1
        assert occ[0][0][1]["danger_level"] == "safe"
