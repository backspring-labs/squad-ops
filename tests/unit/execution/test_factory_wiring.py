...
def test_default_and_absent_config_yield_the_noop_sandbox(tmp_path):
    ...
    default = create_execution_sandbox(ExecutionConfig(workspace_root=tmp_path))
...
def test_docker_provider_without_image_raises(tmp_path):
    ...
    config = ExecutionConfig(provider="docker", workspace_root=tmp_path)
    with pytest.raises(ValueError, match="IMAGE is required"):
        create_execution_service(config)
...
def test_unknown_provider_raises(tmp_path):
    ...
    config = ExecutionConfig(provider="dokcer", workspace_root=tmp_path)
...
def test_service_token_passthrough_for_plain_values(tmp_path):
    ...
    config = ExecutionConfig(workspace_root=tmp_path, service_token="tok-123")
