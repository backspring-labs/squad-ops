...
def _local_config(**overrides):
    """Build a valid local KeycloakOperationalConfig with defaults."""
    defaults = {
...
def _staging_config(**overrides):
    """Build a valid staging KeycloakOperationalConfig."""
    defaults = {
...
def _prod_config(**overrides):
    """Build a valid prod KeycloakOperationalConfig."""
    defaults = {
        "realm": "squadops-prod",
        "base_url": "http://squadops-keycloak:8080",
...
class TestKeycloakOperationalConfigValid:
    ...
    def test_local_config_all_defaults(self):
        cfg = _local_config()
    ...
    def test_staging_config_valid(self):
        cfg = _staging_config()
        assert cfg.realm == "squadops-staging"
    ...
    def test_prod_config_valid(self):
        cfg = _prod_config()
        assert cfg.realm == "squadops-prod"
...
class TestStagingProdValidation:
    """Staging/prod realms enforce stricter constraints."""

    def test_staging_without_db_dsn_fails(self):
        with pytest.raises(ValueError, match="db_dsn required for staging/prod"):
            _staging_config(db_dsn=None)

    def test_prod_without_db_dsn_fails(self):
        with pytest.raises(ValueError, match="db_dsn required for staging/prod"):
            _prod_config(db_dsn=None)

    def test_staging_proxy_mode_none_fails(self):
    ...
    def test_prod_proxy_mode_none_fails(self):
        with pytest.raises(ValueError, match="proxy_mode must not be 'none'"):
            _prod_config(
                proxy_mode="none",
                external_tls_termination=False,
    ...
    def test_staging_hostname_strict_false_fails(self):
        with pytest.raises(ValueError, match="hostname_strict must be true"):
            _staging_config(hostname_strict=False)

    def test_staging_public_url_http_fails(self):
        with pytest.raises(ValueError, match="public_url must use HTTPS"):
            _staging_config(public_url="http://auth.staging.squadops.example")

    def test_prod_public_url_http_fails(self):
        with pytest.raises(ValueError, match="public_url must use HTTPS"):
            _prod_config(public_url="http://auth.squadops.example")

    def test_staging_base_url_localhost_fails(self):
        with pytest.raises(ValueError, match="base_url must not reference loopback"):
            _staging_config(base_url="http://localhost:8180")

    def test_staging_base_url_127_fails(self):
        with pytest.raises(ValueError, match="base_url must not reference loopback"):
            _staging_config(base_url="http://127.0.0.1:8180")

    def test_staging_base_url_ipv6_loopback_fails(self):
        with pytest.raises(ValueError, match="base_url must not reference loopback"):
            _staging_config(base_url="http://[::1]:8180")
...
class TestAuthConfigKeycloak:
    ...
    def test_auth_config_with_keycloak_section(self):
        cfg = AuthConfig(
            ...
            provider="keycloak",
            oidc=OIDCConfig(
                issuer_url="http://kc:8080/realms/squadops-local",
        ...
        assert cfg.keycloak.realm == "squadops-local"
...
class TestRealmLintFunction:
    ...
    def _valid_staging_realm(self) -> dict:
        """Return a minimal valid staging realm dict."""
        return {
            "realm": "squadops-staging",
            ...
            "clients": [
                {
                    ...
                    "redirectUris": ["https://console.staging.example/*"],
                    "webOrigins": ["https://console.staging.example"],
                }
            ],
        }

    def test_valid_staging_passes(self):
        errors = self._lint(self._valid_staging_realm())
        assert errors == []

    def test_missing_revoke_refresh_token(self):
        realm = self._valid_staging_realm()
    ...
    def test_prod_ssl_required_all(self):
        realm = self._valid_staging_realm()
        realm["realm"] = "squadops-prod"
        realm["sslRequired"] = "external"  # should be "all" for prod
        errors = self._lint(realm)
