"""Unit tests for development profile registry (SIP-0072 Phase 1).

Tests DevelopmentProfile dataclass, get_development_profile() lookup, constants,
...
class TestTokenBudgetFields:
    ...
    def test_fullstack_higher_than_cli(self):
        cli = get_development_profile("python_cli")
        fs = get_development_profile("fullstack_fastapi_react")
...
class TestRegistryCompleteness:
    # #822: `assert len(DEVELOPMENT_PROFILES) == 4` was removed rather than bumped to 5. A count
    ...
    # is pinned in tests/unit/cycles/test_stack_development_profile.py (#832).

    def test_all_are_capability_instances(self):
        for name, cap in DEVELOPMENT_PROFILES.items():
            assert isinstance(cap, DevelopmentProfile), f"{name} is not a DevelopmentProfile"
...
class TestFullstackPortableIntegration:
    ...
    @pytest.fixture
    def guidance(self):
        return get_development_profile("fullstack_fastapi_react").file_structure_guidance
