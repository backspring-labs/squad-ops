...
class TestSqlTasksAdapter:
    ...
    async def test_list_tasks(self, adapter, mock_db_pool):
        ...
        filters = TaskFilters(
            cycle_id="cycle-001", agent="test-agent"
    ...
    @pytest.mark.unit
    @pytest.mark.asyncio
    async def test_list_tasks_for_cycle_id(self, adapter, mock_db_pool):
        ...
        assert len(tasks) == 1
        assert tasks[0].cycle_id == "cycle-001"  # SIP-0048: renamed from ecid
        conn.fetch.assert_called_once()
    ...
    async def test_create_flow(self, adapter, mock_db_pool):
        ...
        flow = await adapter.create_flow(
            "cycle-001",  # SIP-0048: renamed from ecid
            "pid-001",
            meta={
                "run_type": "warmboot",
                "title": "Test Flow",
        ...
        assert flow.cycle_id == "cycle-001"  # SIP-0048: renamed from ecid
    ...
    async def test_update_task_status(self, adapter, mock_db_pool):
        ...
        result = await adapter.update_task_status(
            "test-001", "completed", 100.0, None, "test-agent"
        )
...
class TestTasksModels:
    ...
    def test_task_create_model(self):
        ...
        assert task_create.cycle_id == "cycle-001"  # SIP-0048: renamed from ecid
    ...
    def test_task_filters_model(self):
        ...
        assert filters.cycle_id == "cycle-001"  # SIP-0048: renamed from ecid
...
class TestDTOPurity:
    ...
    async def test_list_tasks_returns_dto_list(self, adapter, mock_db_pool):
        ...
        filters = TaskFilters(cycle_id="cycle-001")  # SIP-0048: renamed from ecid
    ...
    async def test_get_task_summary_returns_dto(self, adapter, mock_db_pool):
        ...
        result = await adapter.get_task_summary("cycle-001")  # SIP-0048: renamed from ecid
    ...
    async def test_get_flow(self, adapter, mock_db_pool):
        ...
        flow = await adapter.get_flow("cycle-001")  # SIP-0048: renamed from ecid
    ...
    async def test_list_flows(self, adapter, mock_db_pool):
        ...
        mock_row2.__getitem__ = lambda self, k: {
            "cycle_id": "cycle-002",  # SIP-0048: renamed from ecid
            "pid": "pid-002",
            "run_type": "standard",
            "title": "Flow 2",
            "status": "completed",
        ...
        assert len(flows) == 2
        assert flows[0].cycle_id == "cycle-001"  # SIP-0048: renamed from ecid
        assert flows[1].cycle_id == "cycle-002"  # SIP-0048: renamed from ecid
    ...
    async def test_add_artifact(self, adapter, mock_db_pool):
        ...
        artifact = Artifact(type="file", path="/path/to/file", description="Test artifact")
