"""Tests for the SIP-0070 pulse-verification path, owned by
``PulseBoundaryRunner`` since SIP-0097 slice 4 (moved from
test_dispatched_flow_executor.py — assertions unchanged).

Covers milestone/cadence boundary evaluation, telemetry, record
persistence, and the SIP-0086 bounded repair loop. Most tests drive the
pulse path end-to-end through the executor (which composes the default
runner); ``TestPulseBoundaryRunnerStandalone`` constructs the collaborator
...
# ---------------------------------------------------------------------------
# Fixtures — mirrors of the executor test file's module fixtures (the pulse
# tests drive execute_run end-to-end; FakeReplyRouter + `reply_router` live
# in conftest.py)
# ---------------------------------------------------------------------------
...
class TestPulseBoundaryRunnerStandalone:
    """Drive the pulse path on a directly-constructed PulseBoundaryRunner
    with scripted callables — no executor anywhere."""

    def _make_runner(self, registry=None, bus=None):
    ...
    async def test_boundary_verification_persists_records_and_ledgers(self, cycle):
        """A passing boundary persists every record via the registry, appends
        one ledger entry with the decision and per-suite outcomes, and emits
        the boundary-reached/suite-evaluated/boundary-decided events."""
        from squadops.cycles.pulse_models import PulseVerificationRecord

        runner, registry, bus = self._make_runner()
        ledger = RunLedger()
        ...
        with patch(
            ...
            decision, out_records = await runner._run_boundary_verification(
                suites=[suite],
                boundary_id="post_dev",
                cadence_interval_id=1,
                run_id="run_001",
        ...
        registry.record_pulse_verification.assert_awaited_once_with("run_001", records[0])
        assert len(ledger.pulse_entries) == 1
        entry = ledger.pulse_entries[0]
        assert entry["boundary_id"] == "post_dev"
        assert entry["decision"] == "pass"
        assert entry["suites"] == [{"suite_id": "suite_a", "outcome": "pass"}]
    ...
    async def test_exhaustion_raises_execution_error_after_max_repairs(self, cycle):
        """Error path: suites that keep failing exhaust the repair budget —
        PULSE_REPAIR_EXHAUSTED is emitted and the same _ExecutionError the
        run loop converts to FAILED is raised, naming the boundary."""
        from adapters.cycles.execution_errors import _ExecutionError
        from squadops.cycles.pulse_models import PulseVerificationRecord

        runner, _registry, bus = self._make_runner()

        def failing_records(*, suites, boundary_id, cadence_interval_id, run_id, **_kw):
            return [
                PulseVerificationRecord(
                    suite_id=s.suite_id,
        ...
        async def fake_run_pv(**kwargs):
            return failing_records(**kwargs)

        suite = MagicMock(suite_id="suite_a")
        with (
            patch(
                "adapters.cycles.pulse_boundary_runner.run_pulse_verification",
                side_effect=fake_run_pv,
            ),
            pytest.raises(_ExecutionError, match="VERIFICATION_EXHAUSTED.*boundary_x"),
        ):
        ...
        emitted = [c.args[0] for c in bus.emit.call_args_list]
        assert EventType.PULSE_REPAIR_STARTED in emitted
        assert EventType.PULSE_REPAIR_EXHAUSTED in emitted

    def test_last_task_always_closes_cadence_boundary(self):
        """evaluate_pulse_boundaries: the final plan task closes the boundary
        even when task-count and time budgets are untouched (off-by-one here
        silently drops the run's final cadence evaluation)."""
        from squadops.cycles.pulse_models import CadencePolicy

        runner, _registry, _bus = self._make_runner()
        plan = [MagicMock(), MagicMock(), MagicMock()]
        cadence = CadencePolicy(max_pulse_seconds=9999, max_tasks_per_pulse=100)

        import time as _time

        now = _time.monotonic()
        assert not runner.evaluate_pulse_boundaries(
            task_idx=1, plan=plan, cadence_task_count=1, cadence_start_time=now, cadence=cadence
        )
