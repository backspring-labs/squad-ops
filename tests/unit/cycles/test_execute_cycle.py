...
def _gate_decision(gate_name: str, decision: str, notes: str | None = None) -> GateDecision:
    return GateDecision(
        gate_name=gate_name,
        decision=decision,
        decided_by="user",
...
class TestSingleWorkloadFastPath:
    ...
    async def test_missing_workload_sequence_delegates(self, executor, mock_registry):
    ...
    async def test_empty_workload_sequence_delegates(self, executor, mock_registry):
...
class TestMultiWorkloadOrchestration:
    ...
    async def test_two_workloads_creates_second_run(self, executor, mock_registry, mock_event_bus):
    ...
    async def test_three_workloads_creates_all_runs(self, executor, mock_registry, mock_event_bus):
...
class TestInterWorkloadGates:
    ...
    async def test_rejected_gate_stops_orchestration(self, executor, mock_registry, mock_event_bus):
    ...
    async def test_auto_gate_skips_polling(self, executor, mock_registry, mock_event_bus):
    ...
    async def test_mixed_named_and_auto_gates(self, executor, mock_registry, mock_event_bus):
...
class TestDuplicateRunGuard:
    ...
    async def test_existing_next_run_reused(self, executor, mock_registry, mock_event_bus):
...
class TestEventEmission:
    ...
    async def test_completed_workload_emits_workload_completed(
        ...
        first_completed = [
            c for c in mock_event_bus.emit.call_args_list if c[0][0] == EventType.WORKLOAD_COMPLETED
        ][0]
        assert first_completed[1]["payload"]["terminal_status"] == "completed"

    async def test_gate_emits_gate_awaiting(self, executor, mock_registry, mock_event_bus):
    ...
    async def test_advancement_emits_workload_advanced(
        ...
        advanced_call = [
            c for c in mock_event_bus.emit.call_args_list if c[0][0] == EventType.WORKLOAD_ADVANCED
        ][0]
...
class TestPollInterWorkloadGate:
    ...
    async def test_returns_decision_when_found(self, executor, mock_registry):
        ...
        result = await executor._poll_inter_workload_gate("run_001", cycle, "progress_plan_review")

        assert result.gate_name == "progress_plan_review"
        assert result.decision == GateDecisionValue.APPROVED

    async def test_polls_until_decision_arrives(self, executor, mock_registry):
    ...
    async def test_cancellation_during_poll_raises(self, executor, mock_registry):
        ...
        with pytest.raises(_CancellationError):
            await executor._poll_inter_workload_gate("run_001", cycle, "progress_plan_review")
...
class TestCreateNextWorkloadRun:
    ...
    async def test_run_number_increments_from_max(self, executor, mock_registry):
...
class TestBuildForwardingOverrides:
    ...
    async def test_promoted_artifacts_forwarded_as_prior_workload_refs(self, executor):
    ...
    async def test_planning_run_forwards_plan_artifact_refs(self, executor):
        ...
        executor._artifact_vault.list_artifacts.return_value = [doc1, doc2]
    ...
    async def test_planning_run_includes_control_implementation_plan(self, executor):
        ...
        executor._artifact_vault.list_artifacts.return_value = [doc, manifest, noise]

        result = await executor._build_forwarding_overrides(cycle, completed)

        assert result["plan_artifact_refs"] == ["art_doc", "art_manifest"]
        assert "art_report" not in result["plan_artifact_refs"]

    async def test_implementation_run_forwards_impl_run_id(self, executor):
    ...
    async def test_scalar_override_not_overwritten(self, executor):
    ...
    async def test_list_override_merged_and_deduped(self, executor):
