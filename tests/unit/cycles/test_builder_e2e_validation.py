...
class TestRoutingDiagnosticsInMetadata:
    ...
    def test_builder_build_has_routing_reason(self, run, builder_profile):
        cycle = _make_cycle({
            "build_tasks": ["builder.build", "qa.build_validate"],
        })
        envelopes = generate_task_plan(cycle, run, builder_profile)
        builder_env = next(e for e in envelopes if e.task_type == "builder.build")
        assert builder_env.metadata["routing_reason"] == ROUTING_BUILDER_PRESENT

    def test_qa_build_validate_has_routing_reason(self, run, builder_profile):
        cycle = _make_cycle({
            "build_tasks": ["builder.build", "qa.build_validate"],
        })
        envelopes = generate_task_plan(cycle, run, builder_profile)
        qa_env = next(e for e in envelopes if e.task_type == "qa.build_validate")
...
class TestBuilderPlanShape:
    ...
    def test_build_step_sequence(self, run, builder_profile):
        ...
        expected = [step[0] for step in BUILDER_BUILD_TASK_STEPS]
        assert build_types == expected

    def test_builder_build_assigned_to_bob(self, run, builder_profile):
        cycle = _make_cycle({
            "build_tasks": ["builder.build", "qa.build_validate"],
        })
        envelopes = generate_task_plan(cycle, run, builder_profile)
    ...
    def test_build_only_mode(self, run, builder_profile):
        ...
        assert [e.task_type for e in envelopes] == [
            "builder.build", "qa.build_validate",
        ]
...
class TestHandlerRegistryE2E:
    ...
    def test_builder_build_in_full_registry(self):
        registry = create_handler_registry()
        assert "builder.build" in registry.list_capabilities()

    def test_development_build_in_full_registry(self):
        registry = create_handler_registry()
        assert "development.build" in registry.list_capabilities()

    def test_qa_build_validate_in_full_registry(self):
        registry = create_handler_registry()
        assert "qa.build_validate" in registry.list_capabilities()
    ...
    def test_builder_and_dev_build_coexist(self):
        """Both builder.build and development.build registered without conflict."""
        registry = create_handler_registry()
...
class TestYAMLProfileBuilderIntegration:
    ...
    async def test_builder_profile_plan_emits_builder_build(self, provider, run):
        profile = await provider.get_profile("full-squad-with-builder")
        cycle = _make_cycle({
            "build_tasks": ["builder.build", "qa.build_validate"],
        })
