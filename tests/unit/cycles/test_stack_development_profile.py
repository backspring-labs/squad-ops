...
`development_profile` selects the dev agent's prompt text, `expected_extensions`, `source_filter`
...
- **a fullstack cycle falling back to `python_cli`** because `development_profile` was merely
...
from squadops.capabilities.development_profiles import DEVELOPMENT_PROFILES
from squadops.capabilities.scaffold import (
    ScaffoldStack,
    development_profile_for,
    resolve_development_profile,
)
from squadops.cycles.preflight import stack_development_profile_decision
...
def test_a_scaffoldable_build_profile_derives_the_capability():
    ...
    simply omitted `development_profile` would have been handed CLI prompts — an omission, not a
    contradiction, and therefore invisible to any equality check."""
    assert resolve_development_profile({"build_profile": _STACK}) == _STACK


def test_agreement_is_accepted_unchanged():
    assert (
        resolve_development_profile({"build_profile": _STACK, "development_profile": _STACK})
        == _STACK
    )
...
def test_contradiction_resolves_to_none_rather_than_a_winner():
    ...
    assert (
        resolve_development_profile({"build_profile": _STACK, "development_profile": "react_app"})
        is None
    )
...
@pytest.mark.parametrize(
    ("config", "expected"),
    [
        ({"development_profile": "python_cli"}, "python_cli"),
        ({"development_profile": "react_app"}, "react_app"),
        (
            {"build_profile": "not_a_registered_stack", "development_profile": "python_cli"},
            "python_cli",
        ),
        ({}, ""),
        (None, ""),
...
def test_a_cycle_with_no_scaffoldable_stack_is_untouched(config, expected):
    ...
    assert resolve_development_profile(config) == expected
...
def test_a_contradicting_cycle_is_rejected_at_create():
    ...
    decision = stack_development_profile_decision(
        {"build_profile": _STACK, "development_profile": "react_app"}
    )

    assert decision.rejected
    assert decision.blocking[0].code == "stack_development_profile_mismatch"
...
@pytest.mark.parametrize(
    "config",
    [
        {"build_profile": _STACK, "development_profile": _STACK},
...
def test_valid_configurations_are_not_blocked(config):
    assert not stack_development_profile_decision(config).rejected
...
def test_every_registered_stack_names_a_capability_that_exists():
    ...
    for name, stack in scaffold._STACKS.items():
        assert stack.development_profile, f"stack {name!r} declares no development_profile"
        assert stack.development_profile in DEVELOPMENT_PROFILES, (
            f"stack {name!r} names development_profile {stack.development_profile!r}, which is not registered"
        )
...
def test_a_second_stack_does_not_inherit_the_first_stacks_capability(monkeypatch):
    ...
    assert development_profile_for("node_ts") == ""
    assert development_profile_for(_STACK) == _STACK
    assert resolve_development_profile(
        {"build_profile": "node_ts", "development_profile": "python_cli"}
    ) == ("python_cli"), "an undeclaring stack must not silently claim the config's value is wrong"
