  ...
  it('POST /api/runs -> 201 [vc-probe-api-runs]', async () => {
    const res = await (routeApiRuns.POST as Handler)(
      new Request('http://test/api/runs', {
        ...
        body: JSON.stringify({"title": "sample", "datetime": "2026-08-01T08:00:00", "meeting_location": "sample"}),
      }),
    )
    ...
    const expectShape = (o: any) => {
      for (const k of ["id", "title", "datetime", "meeting_location", "participants", "created_at"]) expect(o?.[k]).not.toBeUndefined()
...
expect(body.meeting_location).toBe('sample')
