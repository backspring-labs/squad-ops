...
| `1-6-6-react-roll-4-interface_manifest.yaml` | `cyc_ac15b6c6209f` / `art_0e3669d15b93` (the framing run's manifest) | the manifest that declared `Run` as the create's response and `RunDetail` as the read-by-id's — two single-object shapes of one stored thing; #1112's edge, and the #1087 stack-#1 replay |
| `1-6-6-react-roll-4-routes-fill.py.txt` | `cyc_ac15b6c6209f` / `art_a8c0cdaa1b93`, the accepted run's final `backend/routes.py` (2026-08-27 22:55:06Z) | the fill that imported `run_store, run_detail_store, participant_store` and kept all three in sync by hand — what a store per declared entity costs a correct app. `.py.txt` so it is neither collected nor linted |
| `1-7-0-react-roll-6-interface_manifest.yaml` | `cyc_cb49b16c2fa6` / `art_9f21fee57fd7` (the gating pair's React manifest) | the accepted roll's manifest — `Run` created and read by id, `Participant` embedded; the root-table store's no-change control on the root |
