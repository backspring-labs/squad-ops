...
def check_service_health(
    ...
    for attempt in range(1, retries + 1):
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                if result == 0:
                    if attempt > 1:
                        print(
                            f"✅ {service_name} is healthy on {host}:{port} (succeeded on attempt {attempt})"
                        )
                    else:
        ...
        if attempt < retries:
            print(
                f"⚠️  {service_name} health check attempt {attempt}/{retries} failed on {host}:{port}, retrying..."
            )
            time.sleep(2)  # Brief pause between retries

    print(
        f"❌ {service_name} failed health check on {host}:{port} after {retries} attempts ({timeout}s each)"
    )
    print(
        f"   Troubleshooting: Check if {service_name} is running with 'docker ps' or 'systemctl status {service_name.lower()}'"
    )
    return False
...
def check_rabbitmq_management(host: str, port: int, timeout: int = 30, retries: int = 3) -> bool:
    ...
    for attempt in range(1, retries + 1):
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                if response.status_code == 200:
                    if attempt > 1:
                        print(
                            f"✅ RabbitMQ management is healthy on {host}:{port} (succeeded on attempt {attempt})"
                        )
                    else:
        ...
        if attempt < retries:
            print(
                f"⚠️  RabbitMQ management health check attempt {attempt}/{retries} failed on {host}:{port}, retrying..."
            )
            time.sleep(2)  # Brief pause between retries

    print(
        f"❌ RabbitMQ management failed health check on {host}:{port} after {retries} attempts ({timeout}s each)"
    )
    print(
        f"   Troubleshooting: RabbitMQ management is optional but recommended. Check with 'docker ps' or visit http://{host}:{port}"
    )
    return False
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            for agent in agents:
                ...
                if not info["running"]:
                    print(
                        f"❌ Agent {agent} ({info['container_name']}) is not running: {info['status']}"
                    )
                    all_healthy = False
                    failed_agents.append(agent)
                else:
                    if attempt > 1:
                        print(
                            f"✅ Agent {agent} ({info['container_name']}) is running: {info['status']} (succeeded on attempt {attempt})"
                        )
                    else:
                        print(
                            f"✅ Agent {agent} ({info['container_name']}) is running: {info['status']}"
                        )
            ...
            if attempt < retries:
                print(
                    f"⚠️  Agent container check attempt {attempt}/{retries} failed for: {failed_agents}, retrying..."
                )
                time.sleep(2)  # Brief pause between retries
            else:
                print(f"❌ Agent containers failed health check after {retries} attempts")
                print(
                    "   Troubleshooting: Start agents with 'docker-compose up -d max neo' or check logs with 'docker logs squadops-max'"
                )
                return False
            ...
            if attempt < retries:
                print(
                    f"⚠️  Agent container check attempt {attempt}/{retries} failed with error: {e}, retrying..."
                )
                time.sleep(2)
            else:
...
def check_required_services():
    ...
    # Check agent containers (optional - skip if SKIP_AGENT_CHECK env var is set)
    # Adapter-only tests don't need agents, so this can be bypassed
    if os.getenv("SKIP_AGENT_CHECK", "").lower() in ("1", "true", "yes"):
        print("\n📊 Skipping agent container check (SKIP_AGENT_CHECK=1)")
        service_status["agent_containers"] = True  # Assume healthy for service summary
    else:
        ...
        if not agents_healthy:
            ...
            print("   💡 Or set SKIP_AGENT_CHECK=1 to skip this check for adapter-only tests")
            pytest.skip(
                "Agent containers (Max/Neo) are not running. Start with 'docker-compose up -d max neo' or set SKIP_AGENT_CHECK=1"
            )
...
async def ensure_agents_running_fixture():
    ...
    if not success:
        pytest.skip(
            "Failed to ensure agent containers are running. Check Docker and agent configuration."
        )
