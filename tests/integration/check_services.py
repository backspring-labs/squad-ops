...
class ServiceChecker:
    ...
    def check_service_health(
        self, service_name: str, host: str, port: int, timeout: int = 30
    ) -> tuple[bool, str]:
        """
    ...
    def check_ollama(
        self, url: str = "http://localhost:11434", timeout: int = 30
    ) -> tuple[bool, str]:
        """
    ...
    def check_all_services(self, include_optional: bool = False) -> dict[str, tuple[bool, str]]:
        ...
        # Optional services
        if include_optional:
            results["rabbitmq_management"] = self.check_rabbitmq_management(
                "localhost", 15672, timeout=10
            )
...
def main():
    ...
    parser.add_argument(
        "--service", "-s", help="Check specific service (postgres, redis, rabbitmq, agents, ollama)"
    )
    parser.add_argument(
        "--include-optional",
        action="store_true",
        help="Include optional services (RabbitMQ Management, Ollama)",
    )
    ...
    if args.service:
        ...
        if service == "postgres":
            ...
            print(
                "Available services: postgres, redis, rabbitmq, rabbitmq-management, agents, ollama"
            )
            return 1
