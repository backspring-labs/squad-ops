...
def audit_registry() -> dict[str, Any]:
                ...
                issues["data_quality"].append(
                    {
                        ...
                        "sip_number": sip_number,
                        "entries": [
                            {"title": e.get("title"), "variant": e.get("variant")} for e in entries
                        ],
            ...
            issues["critical"].append(
                {
                    ...
                    "sip_uid": sip_uid,
                    "entries": [
                        {"sip_number": e.get("sip_number"), "title": e.get("title")}
                        for e in entries
                    ],
    ...
    all_numbers = {
        sip.get("sip_number")
        for sip in registry.get("sips", [])
        if sip.get("sip_number") is not None
    }
...
def print_audit_report(audit_results: dict[str, Any]) -> None:
    ...
    if missing_file_count > 0:
        recommendations.append(
            f"Remove {missing_file_count} registry entry/entries for non-existent files"
        )
    ...
    duplicate_variant_count = sum(
        1 for i in issues["data_quality"] if i["type"] == "duplicate_number_missing_variant"
    )
    if duplicate_variant_count > 0:
        recommendations.append(
            f"Add variant fields for {duplicate_variant_count} duplicate number group(s)"
        )
