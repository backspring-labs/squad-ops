...
def validate_transition(current_status: str, new_status: str) -> tuple[bool, str]:
    ...
    if new_status not in VALID_TRANSITIONS[current_status]:
        return (
            False,
            f"Invalid transition: {current_status} → {new_status}. Allowed: {VALID_TRANSITIONS[current_status]}",
        )
...
def find_duplicate_sip_files(
    sip_uid: str | None, sip_number: int | None, canonical_file: Path, registry: dict[str, Any]
) -> list[Path]:
    """
    ...
    canonical_original_filename = (
        canonical_metadata.get("original_filename") if canonical_metadata else None
    )
    canonical_title = (
        normalize_title(canonical_metadata.get("title", "")) if canonical_metadata else None
    )
    ...
    for lifecycle_dir in lifecycle_dirs:
        ...
        for sip_file in lifecycle_dir.glob("*.md"):
            ...
            if sip_uid and metadata.get("sip_uid") == sip_uid:
                ...
                for sip_entry in registry.get("sips", []):
                    if (
                        sip_entry.get("sip_uid") == sip_uid
                        and sip_entry.get("status") != canonical_status
                    ):
                        is_legitimate = True
                        break
            ...
            # Match by original_filename
                ...
                canonical_original_filename
                and metadata.get("original_filename") == canonical_original_filename
            ):
                is_duplicate = True
    ...
    if current_status == "proposed" and new_status == "accepted":
        ...
        try:
            ...
            try:
                ...
                print("   Destination exists and is valid, continuing...")
        ...
        if original_file_path.exists():
            ...
            try:
                ...
                print("   ✅ Removed orphaned source file")
        ...
        if orphaned_by_name.exists() and orphaned_by_name.resolve() != new_path.resolve():
            print(
                f"   Found orphaned file by original name: {orphaned_by_name.relative_to(REPO_ROOT)}"
            )
            try:
        ...
        if sip_number is None:
            print(
                f"Error: SIP must have a number to transition from {current_status} to {new_status}"
            )
            return False

        # Find SIP in registry
        registry_entry = find_sip_in_registry(
            registry, sip_number=sip_number, sip_uid=metadata.get("sip_uid")
        )
        ...
        if duplicates:
            print(
                f"   Found {len(duplicates)} duplicate(s) for SIP-{sip_number:04d} (sip_uid: {metadata.get('sip_uid', 'N/A')}):"
            )
            for dup in duplicates:
                print(f"   - {dup.relative_to(REPO_ROOT)}")
        ...
        if (
            expected_dir
            and expected_dir not in sip_file.parents
            and sip_file.parent != expected_dir
        ):
            print(
                f"Warning: SIP file location ({sip_file.parent}) doesn't match current status ({current_status})"
            )
