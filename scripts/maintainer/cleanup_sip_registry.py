            ...
            if metadata and metadata.get('sip_number') is not None:
                ...
                if folder_status:
                    ...
                    entry = {
                        ...
                        'path': registry_path,
                        'created_at': metadata.get('created_at') or datetime.now().strftime("%Y-%m-%dT%H:%M:%SZ"),
                        'updated_at': datetime.now().strftime("%Y-%m-%dT%H:%M:%SZ"),
                    }
                    
                    # Add variant if needed (check for duplicates)
                    existing_with_number = [s for s in registry.get('sips', []) if s.get('sip_number') == sip_number]
...
def main():
    ...
    parser.add_argument('--fix-missing', action='store_true', help='Remove registry entries for non-existent files')
    parser.add_argument('--fix-timestamps', action='store_true', help='Fix invalid timestamp formats')
    parser.add_argument('--fix-corrupted-timestamps', action='store_true', help='Fix corrupted created_at timestamps by syncing from file metadata or using fallback dates')
    parser.add_argument('--fix-variants', action='store_true', help='Add variant fields to duplicate numbers')
    parser.add_argument('--fix-uids', action='store_true', help='Fix duplicate sip_uids by syncing with file metadata')
    parser.add_argument('--add-orphaned', action='store_true', help='Add orphaned numbered files to registry')
    parser.add_argument('--all', action='store_true', help='Fix all issues')
    
    args = parser.parse_args()
    
    if not any([args.fix_missing, args.fix_timestamps, args.fix_corrupted_timestamps, args.fix_variants, args.fix_uids, args.add_orphaned, args.all]):
        print("No cleanup actions specified. Use --all to fix everything, or specify individual fixes.")
        print("\nAvailable fixes:")
        print("  --fix-missing              Remove registry entries for non-existent files")
        print("  --fix-timestamps           Fix invalid timestamp formats")
        print("  --fix-corrupted-timestamps Fix corrupted created_at timestamps by syncing from file metadata")
