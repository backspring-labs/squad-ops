...
DDD migration compatibility:
- Source `agents/`, `infra/`, and `config/` from `_v0_legacy/`.
- Overlay `src/squadops/{core,ports}` into `dist/infra/` so legacy containers can
  progressively bridge to 1.0 Secrets/Persistence ports while keeping role logic.
"""
...
def write_text_file(dst: Path, content: str, description: str = "") -> None:
    """Write a UTF-8 text file, creating parents as needed."""
    dst.parent.mkdir(parents=True, exist_ok=True)
    dst.write_text(content, encoding="utf-8")
    if description:
        logger.debug(f"Wrote {description}: {dst}")


def ensure_ddd_bridge(dist_dir: Path) -> None:
    """
    Ensure legacy import paths can bridge into the DDD core.

    Goal: legacy imports like `from infra.secrets.manager import SecretManager` keep working,
    but the implementation delegates parsing/resolution to the DDD core copied into
    `dist/infra/core/secrets.py`.
    """
    infra_dir = dist_dir / "infra"
    secrets_dir = infra_dir / "secrets"
    secrets_dir.mkdir(parents=True, exist_ok=True)

    # Keep legacy `SecretManager.from_config(...)` API required by infra.config.loader,
    # but delegate secret parsing/replacement to DDD core SecretManager.
    manager_py = '''\
"""
DDD bridge: infra.secrets.manager

Preserves legacy `SecretManager.from_config(...)` and `SecretManager._has_secret_references(...)`
APIs required by `infra.config.loader`, but delegates secret parsing/resolution to the
DDD core implementation located at `infra.core.secrets`.
"""
...
from infra.core.secrets import SecretManager as _CoreSecretManager


class SecretManager(_CoreSecretManager):
    """Legacy-compatible SecretManager that delegates to DDD core."""
    ...
    def from_config(cls, secrets_config: SecretsConfig) -> "SecretManager":
        ...
        # Provider creation stays in this bridge layer (keeps core pure).
        if secrets_config.provider == "env":
            from infra.secrets.env_provider import EnvProvider

            env_prefix = secrets_config.env_prefix or "SQUADOPS_"
    ...
    @staticmethod
    def _has_secret_references(
        config_dict: dict[str, Any], exclude_keys: list[str] | None = None
    ) -> bool:
        return _CoreSecretManager.has_secret_references(config_dict, exclude_keys=exclude_keys)
'''

    exceptions_py = '''\
"""
DDD bridge: infra.secrets.exceptions

Re-export DDD core exceptions under the legacy import path used throughout infra.
"""

from infra.core.secrets import InvalidSecretReferenceError, SecretNotFoundError, SecretResolutionError

__all__ = ["SecretResolutionError", "SecretNotFoundError", "InvalidSecretReferenceError"]
'''

    provider_py = '''\
"""
DDD bridge: infra.secrets.provider

Expose the DDD port interface as the canonical SecretProvider contract.
"""

from infra.ports.secrets import SecretProvider

__all__ = ["SecretProvider"]
'''

    write_text_file(secrets_dir / "manager.py", manager_py, "DDD bridge secrets manager")
    write_text_file(secrets_dir / "exceptions.py", exceptions_py, "DDD bridge secrets exceptions")
    write_text_file(secrets_dir / "provider.py", provider_py, "DDD bridge secrets provider")
...
def get_git_commit(base_path: Path, build_arg: str | None = None) -> str | None:
    ...
    try:
        result = subprocess.run(
            ["git", "rev-parse", "HEAD"], capture_output=True, text=True, cwd=base_path, timeout=5
        )
...
def generate_agent_info(
    role: str, capabilities: list[str], skills: list[str], build_hash: str
) -> dict[str, Any]:
    """
...
def build_agent_package(role: str, base_path: Path) -> None:
    ...
    # Paths
    legacy_root = base_path / "_v0_legacy"
    if not legacy_root.exists():
        raise FileNotFoundError(f"Legacy root not found: {legacy_root}")

    role_dir = legacy_root / "agents" / "roles" / role
    ...
    for cap_name in capabilities:
        dirs = copy_capability_files(cap_name, legacy_root, dist_dir)
        copied_cap_dirs.update(dirs)

    # Copy capability loader and catalog (always needed)
    copy_directory(
        legacy_root / "agents" / "capabilities" / "loader.py",
        dist_dir / "agents" / "capabilities" / "loader.py",
        "capability loader",
    )
    copy_directory(
        legacy_root / "agents" / "capabilities" / "catalog.yaml",
        dist_dir / "agents" / "capabilities" / "catalog.yaml",
        "capability catalog",
    )
    copy_directory(
        legacy_root / "agents" / "capabilities" / "__init__.py",
    ...
    shared_dirs = [
        ...
        (
            "agents/tasks",
            "task models and adapters",
        ),  # ACI v0.8: Required for TaskEnvelope deserialization
        ...
        (
            "infra",
            "infrastructure modules (config, etc.)",
        ),  # SIP-051: Required for centralized config
    ]

    for rel_path, description in shared_dirs:
        src = legacy_root / rel_path
        dst = dist_dir / rel_path
        copy_directory(src, dst, description)

    # DDD overlay: expose new core + ports inside dist/infra (namespace bridge)
    ddd_root = base_path / "src" / "squadops"
    if ddd_root.exists():
        copy_directory(
            ddd_root / "core",
            dist_dir / "infra" / "core",
            "DDD core (src/squadops/core -> dist/infra/core)",
        )
        copy_directory(
            ddd_root / "ports",
            dist_dir / "infra" / "ports",
            "DDD ports (src/squadops/ports -> dist/infra/ports)",
        )
    else:
        logger.warning(f"DDD root not found, skipping DDD overlay: {ddd_root}")

    # Maintain the legacy import path: infra.secrets.* delegates to DDD core.
    ensure_ddd_bridge(dist_dir)

    # Copy tools directory (needed for dev role)
    if role == "dev":
        copy_directory(legacy_root / "agents" / "tools", dist_dir / "agents" / "tools", "dev tools")

    # Copy skills (role-specific + shared)
    skills_src = legacy_root / "agents" / "skills"
    skills_dst = dist_dir / "agents" / "skills"

    # Copy shared skills
    if (skills_src / "shared").exists():
        copy_directory(skills_src / "shared", skills_dst / "shared", "shared skills")

    # Copy role-specific skills
    if (skills_src / role).exists():
        copy_directory(skills_src / role, skills_dst / role, f"{role} skills")

    # Copy skills __init__ and registry
    if (skills_src / "__init__.py").exists():
        copy_directory(skills_src / "__init__.py", skills_dst / "__init__.py", "skills __init__")
    if (skills_src / "registry.yaml").exists():
        copy_directory(
            skills_src / "registry.yaml", skills_dst / "registry.yaml", "skills registry"
        )

    # Copy config files
    copy_directory(
        legacy_root / "agents" / "capability_bindings.yaml",
        dist_dir / "agents" / "capability_bindings.yaml",
        "capability bindings",
    )

    # Copy role-specific files
    copy_directory(role_dir / "agent.py", dist_dir / "agent.py", "agent entry point")
    copy_directory(
        role_dir / "config.yaml",
        dist_dir / "agents" / "roles" / role / "config.yaml",
        "agent config",
    )
    copy_directory(role_dir / "requirements.txt", dist_dir / "requirements.txt", "requirements")

    # Copy registry.yaml (needed for role context)
    copy_directory(
        legacy_root / "agents" / "roles" / "registry.yaml",
        dist_dir / "agents" / "roles" / "registry.yaml",
        "role registry",
    )

    # Copy config directory
    copy_directory(legacy_root / "config", dist_dir / "config", "config directory")
    ...
    agent_info = generate_agent_info(
        role=role, capabilities=capabilities, skills=skills, build_hash=build_hash
    )
    ...
    logger.info(
        f"   Package size: {sum(f.stat().st_size for f in dist_dir.rglob('*') if f.is_file()) / 1024:.1f} KB"
    )
