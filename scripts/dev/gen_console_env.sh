#!/usr/bin/env bash
# gen_console_env.sh — Parse continuum.lock and generate .env.console for Docker build args.
...
LOCK_FILE="${REPO_ROOT}/continuum.lock"
ENV_FILE="${REPO_ROOT}/.env.console"

if [ ! -f "${LOCK_FILE}" ]; then
    echo "ERROR: continuum.lock not found at ${LOCK_FILE}" >&2
    exit 1
fi
...
cat > "${ENV_FILE}" <<EOF
# Auto-generated from continuum.lock — do not edit manually.
