...
**Current**: v1.4.4 — the verification-integrity patch on the Verified Canonical App Build (v1.4.0: dual-lane headline SIP-0099 Contract-First Build Scaffolding, Lane M + SIP-0102 Ephemeral Application Sandbox, Lane S, with SIP-0098 Verification Contracts and SIP-0100 Scaffolded Test Harness completing the arc; v1.4.1: the cut's known-open five-fix ledger; v1.4.2: the correction-aim patch). Exit evidence for the 1.4 arc is unchanged and stands: Functional App Yield **6/6 (100% functional), 5/6 green, five consecutive greens** over a pre-registered unfiltered window on a frozen deploy, every deliverable independently audited. The claim remains precise: the squad implements and verifies a **specified** application contract end-to-end; authoring the interface design from the PRD is the v1.6 headline. Shipped in v1.4.4 — **every verdict is earned**: a failed run carries its exact terminal reason on the run row and in `runs show` (#427), the plan author is only offered builder tasks the config can execute and the gate rejects the rest (#426), a qa task whose declared artifacts can never satisfy `tests_pass` is rejected at authoring instead of burning correction rounds (#715), a skipped-but-authored check is an evidence gap that can never read as passed — required-level when contract-bound (#423), plan-authoring collapse is a gate rejection rather than a silent static-step fallback (#424), the time budget gates every dispatch lane including correction chains (#511), and semantic-memory recall prefilters in-query with a valid cosine score (#571). The whole line was verified on an integrated overnight deploy: validator replays against stored artifacts, a designed-failure probe, and confirmation shakedown shk-5 — green, verdict accepted, zero corrections, with the new nets silent on a well-formed roll. Next: the v1.5 stabilization release.
...
- **6,000+ tests** passing in regression suite
