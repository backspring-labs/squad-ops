...
**Current**: v1.4.3 — the lifecycle-hygiene patch on the Verified Canonical App Build (v1.4.0: dual-lane headline SIP-0099 Contract-First Build Scaffolding, Lane M + SIP-0102 Ephemeral Application Sandbox, Lane S, with SIP-0098 Verification Contracts and SIP-0100 Scaffolded Test Harness completing the arc; v1.4.1: the cut's known-open five-fix ledger; v1.4.2: the correction-aim patch). Exit evidence for the 1.4 arc is unchanged and stands: Functional App Yield **6/6 (100% functional), 5/6 green, five consecutive greens** over a pre-registered unfiltered window on a frozen deploy, every deliverable independently audited. The claim remains precise: the squad implements and verifies a **specified** application contract end-to-end; authoring the interface design from the PRD is the v1.6 headline. Shipped in v1.4.3 — **the loop can't strand or hide**: a cancelled or dead run releases every focus lease it held (cancel routes, executor finalize, and startup sweeps all drive the coordinator, #373/#529), a lease-less stranded `cycle` mode is returned to ambient at startup instead of silently disabling arbitration (#710), a stale finalize can no longer strip a relaunched run's focus (owner-checked release, #712), a D9 activity conflict self-heals by superseding the stale row (#561), acceptance commands resolve `python` to the harness interpreter after the safelist gate (#498), the queue declares only the capabilities it honors (#572), and PII redaction no longer swallows adjacent log fields (#573). All seven proven live in the deploy window: first-boot reaps, a deliberate cancel probe, and the #712 race demonstrated and refused in production shape. Next: the v1.4.4 verification-integrity patch, then the v1.5 stabilization release.
