...
**Current Status**: v1.5.0 — Experiment-ready framework with hexagonal architecture. The 1.5 stabilization line **finishes the 1.4 machinery's promises and extracts the structure that survived five patch lines**: verification integrity is complete end to end (SIP-0096 implemented — every verdict names the evidence that earned it, waivers disclose without rewriting the verdict, wrap-up prose is clamped to the outcome it cites), qa emissions join the typed-acceptance seam so framework checks reach both authoring surfaces, correction evidence carries real tracebacks and named extraction loss, and the executor's context assembly, the planning handler, and the typed-check menu each become a declared registry instead of scattered branches. Feature-free by rule (odd-minor convention): no new contract fields, manifest fields, or squad-facing capabilities — contract v9 and manifest v4 are byte-stable line-wide. The **Verified Canonical App Build** claim from v1.4 stands unchanged: given a PRD and a fully specified interface manifest, the squad implements, verifies, and delivers a working full-stack application — measured at **100% functional yield (6/6 unfiltered rolls, 5/6 fully green, five consecutive)** under a pre-registered fixed-budget protocol with independent sandbox audits (SIP-0099 Contract-First Build Scaffolding + SIP-0102 Ephemeral Application Sandbox, with SIP-0098 Verification Contracts and SIP-0100 Scaffolded Test Harness). Squad-authored interface design remains deliberately out of scope — it is the v1.6 headline. Built on the stabilized 1.3 line (SIP-0097 executor decomposition, push-based agent comms, Agent Runtime State platform SIP-0089), with distributed cycle execution, correction protocol with checkpoint/resume, maintainer-only cycle replay, Postgres persistence, LangFuse observability, Keycloak authentication, CLI tooling, test quality enforcement, and 6,400+ passing tests.
...
**Current**: v1.5.0 — **Finish the Promises, Extract the Proven**, the stabilization minor on the Verified Canonical App Build (v1.4.0: dual-lane headline SIP-0099 Contract-First Build Scaffolding, Lane M + SIP-0102 Ephemeral Application Sandbox, Lane S, with SIP-0098 Verification Contracts and SIP-0100 Scaffolded Test Harness completing the arc; v1.4.1–1.4.4: four hardening patch lines). Exit evidence for the 1.4 arc is unchanged and stands: Functional App Yield **6/6 (100% functional), 5/6 green, five consecutive greens** over a pre-registered unfiltered window on a frozen deploy, every deliverable independently audited. The claim remains precise: the squad implements and verifies a **specified** application contract end-to-end; authoring the interface design from the PRD is the v1.6 headline. Shipped in v1.5.0, in two halves. **Promises finished**: SIP-0096 verification-evidence integrity is implemented, not just promoted — waivers are recorded and disclosed without ever rewriting a verdict (#682), wrap-up prose is clamped to the outcome it cites (#683), and a squad that stops producing evidence is detected as inert rather than read as passing (#684); qa emissions join the typed-acceptance seam so framework-injected checks reach both authoring surfaces (#670); correction evidence carries the app's real traceback (#687) and names its own extraction loss instead of silently truncating (#431); correction termination is progress-aware, so a chain that is moving is never cut short and a chain that is not is never allowed to burn the budget (#435); a test suite whose assertions contradict the frozen contract is a blocking failure (#629). **Structure extracted**: the executor's context assembly is a declared per-task-type registry rather than five tables and three branches (#663), the 1,887-line planning handler is a package split by authoring stage (#331), every typed check declares its own governance metadata in one registry with a generated menu (#730), stranded cycles are detected at boot with their recovery command (#481), and every acceptance verdict names the exact workspace revision it measured (#734). Verified as a line on an integrated deploy: two green confirmation shakedowns (the Gate-2 exit `cyc_ea0b82cfbd17` and the cut `cyc_b07183b3cf5c` — accepted, 36/36 checks, zero corrections), a live replay demonstration, a waiver end-to-end probe, and a replay zero-diff over the stored green corpus. Next: the v1.6 Authorship release.
