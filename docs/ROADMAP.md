...
- **v1.8 — shipped 2026-09-17** — **the Judgement release** (row rewritten 2026-09-12 on the owner's ruling, recorded in `docs/plans/1-8-0-plan.md` §8; previously "the Automation + Learning release" with the Cycle Evaluation Scorecard and the Campaign mechanic co-headlining, billed 2026-08-07 in `docs/plans/post-1-5-roadmap-reconciliation.md`). **Dual-lane headlines: Scoped Code Revision** (Lane M — SIP-0107, accepted at design review 2026-09-13 on PR #1325; ruled 1.8's by the owner 2026-09-09: for an existing artifact the agent describes the smallest reliable revision and the framework realizes it under an explicit write grant, preserving every byte outside the accepted range and verifying exactly the tree it persists; subsumes #1213; #1444's `handle()` split is its first extraction; steps 1–6 ship in 1.8.0 and the default flip is **1.8.1's by design**, landed on the N the 1.8.0 set measures) **+ the Cycle Evaluation Scorecard's 1.8 slice** (Lane S — SIP-0108, accepted at design review 2026-09-13: `CycleAssessment` as a projection over the `CycleOutcome` seam, **one** failure-attribution registry of evidence classes over the vocabularies the code already has, a benchmark registry over the stored counted rolls, and a system-level comparison — the squad against one generalist agent on the same deterministic substrate, cost measured — with one pre-registered window, **re-placed to v1.8.1 by the owner's ruling 2026-09-14** (plan rev 7: the generalist arm changes role routing and the correction path, which deploy B freezes); the console page is SIP-0069's, later). **Rails:** the one cycle-lineage seam (extracted from inert detection; Campaign's objective envelope replaces its implementation in 2.0) and the pre-memory rejection baseline (B1) emitted — thirteen records in the vault, none since 2026-08-23; a zero that says the original Phase-1 proving workload is dormant, not that memory has no value. **Moved off this row by the owner's ruling:** Campaign Orchestration → **v2.0's headline**; Cross-Cycle Memory → **v2.2**, its inert integration rail (the recall port, a NoOp that answers empty, the call site) → **v2.1**, not here. The ordering rule that produced the move stands: grade definitions land before continuation policy — without `CycleAssessment` a continuation policy must invent a stopping rule out of raw checks, and a campaign whose stopping rule reduces to "the cycle completed" runs the false-green class unattended at scale. Gates cleared: SIP-0096 implemented (2026-08-06); the authored-mode baseline banked (1.6.0 4/6, 1.7.3–1.7.5 at 7–8 of 9).
- **v1.9** — stabilization (feature-free by rule): **the close of the 1.x line.** The executor's completion boundary extracted under the 1.7.5 map's rules — `execute_cycle`, `execute_run`, `_reject_invalid_plan_before_workload_gate` (#1507; the seam Campaign lands through in 2.0) — plus the debts the 1.8 plan re-placed by name: #567 (the fenced parser, after Scoped Code Revision settles what a repair emits), #353 (the manifest hash stamped at build), #1031 (the manifest-authoring primer). **2.0's entry condition, and this line's exit criterion:** one named, extracted cycle-completion boundary through which Campaign observes terminal state and requests continuation — Campaign does not begin by decomposing the executor. Substance gates the cut; the 1.9 plan names the rest.
- **v2.0** — **Campaign Orchestration as the headline** (row rewritten 2026-09-12 on the owner's ruling: "solidify the 1.x codeline to make campaigns the 2.0 headline"; previously "compound on the shipped 1.8 scorecard" with Capability-Backed Agents, Campaign capability-augmentation and Self-Improvement + Test Bay as the pillars). The objective envelope + continuation policy (`sips/proposed/SIP-Campaign-Orchestration.md`), consuming 1.8's shipped `CycleAssessment` as its stopping signal — never raw checks — and landing through the completion boundary 1.9 extracts; #316 (the request-profile taxonomy) moves with it; the 1.8 lineage seam is the stable contract whose derivation its objective envelope enriches — no second lineage concept, no consumer migration. **Behind it, sequenced by the 2.0 plan rather than by this row:** the former pillars — Capability-Backed Agents (what an agent *is*), Self-Improvement + Test Bay (the capstone; acts on `CycleAssessment` grades, never raw checks). **v2.2 — Cross-Cycle Memory** (`sips/proposed/SIP-Cross-Cycle-Memory.md`; Phase 1 and then Phase 2, the scoped-memory substrate Capability-Backed Agents consumes; its inert integration rail — the recall port, a NoOp that answers empty, the call site through `plan_rejection_context` — ships in **v2.1**, which also re-reads Phase 1's proving workload against the recurrence evidence of that day).

Each even-minor consumer sits strictly behind the release that earns its trust: author over honest evidence (1.6 behind 1.4); grade over the authored baseline (1.8 behind 1.6); run on its own over trustworthy grades (2.0 behind 1.8 — the boundary between the major versions, ruled 2026-09-12). Odd minors (1.5, 1.7, 1.9) are the stabilization tails between — and they are not junk drawers: 1.5 shipped a named claim, and 1.7 has one (above). The ladder in one line: **1.6 teaches the squad to design, 1.7 makes the seams hold, 1.8 teaches it to judge — and only then, at 2.0, to run on its own.**
...
entity under a write grant, verified as the tree it persists; the default flip, step 7, is
1.8.1's by design) and **the Cycle Evaluation Scorecard's 1.8 slice** (SIP-0108 (a)–(c) — the
per-run summary row, one failure-attribution registry, the pure four-dimension assessment, and 79
historical rolls re-graded; the comparison window with its Solo and Free-Solo arms and the
placement thesis are 1.8.1's, §10g–§10j). Evidence: a pre-registered two-arm set on frozen deploy
`969cd44e`, zero drift under `src/` and `adapters/` to the tag — eight diagnostics reaching their
seams, **FastAPI+React 6 of 6, Next.js+TS 2 of 3, functional 8 of 9**, L1 held on every counted
roll. Stated at the cut: SIP-0107 §39.8's N read **5 of 6** with the qa × Next.js cell empty, and
the owner ruled the line closes with the gap named. The deploy-E set that preceded it found seven
latent seams in one night and fixed six before the measurement (#1586–#1589, #1594, #1596). Plan:
`docs/plans/1-8-0-plan.md` (rev 11). Record: `docs/plans/1-8-0-verification-set-preregistration.md` §10.

### v1.7.5 (2026-09-12) — the three closures
...
**The finding that outranks the rate — corrected 2026-08-25 (record §6):** the cut-time reading was that two of the three failures were the framework wrongly rejecting a working application (#1087). The stored per-round test reports say otherwise: **all three rejected applications failed the frozen response floor of the join probe at every round** (`participants` returned as bare strings or a required field missing where the manifest declares `list[Participant]`), and the phantom-table assertion was a second failure in two of them, never the only one. The loss mode is the repair round not acting on a correct round-0 diagnosis — wrong file emitted, nothing emitted, or a correct repair discarded by a contract-violating fill. #1087 stands as a defect; its yield on this set was zero rolls.
...
### v1.6.1 (2026-08-22) — the correction-loop and framing-information patch line
