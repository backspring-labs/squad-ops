...
Thirteen PRs that change what a record reads, what a roll rejects on, or what an evidence line
...
| 4e | **#1476** | the QA build workspace admits the frozen stylesheet the entry point imports — `index.css` and `globals.css` join `build_support_files`, the slot documented for exactly this (`index.html` already lives there); plus a guard parametrized over the live stack registry that every frozen file the profile itself expects is admitted. **The defect #906 and #1463 introduced, and the reason every one of the first three deploys' six halves was rejected**: `_get_source_artifacts` keeps a file only by `source_filter` extension or `build_support_files` basename, and `.css` was in neither. Baseline 1.7.4: `frontend_build` failed in 0 of 17 records; 1.7.5 before this row: 6 of 6. Invisible until #1468 + #1472 made the reason readable | **deploy A'' is superseded by it — the fourth deploy, justified because it is the first on which the pair can pass.** Read on the pair: `frontend_build` passes on both stacks, or fails with a cause that is not this one. The loaded-check row prints DROPPED on A'' and KEPT on A''' |
| 4d | **#1472** | a failing check row's `reason` survives normalization, and is DERIVED from the row's own evidence when the producer set none — plus a guard, parametrized over the live `FRAMEWORK_CHECKS` registry, that no framework check may fail unexplained. **Found by deploy A''s pair, AFTER #1468 was verified loaded**: the reason was being dropped a layer lower, in `_from_passed_row`. The audit that followed found EIGHT more producers with the same hole, `required_files` among them — a required check able to reject a roll with `reason: ""` exactly as `frontend_build` did. It is #510's fix generalized; #510 closed this for `tests_pass` alone, via the one check with a dedicated normalizer | **deploy A' is superseded by it.** Read on the next pair: a failing framework check states its cause, and the correction loop can tell a partial repair from a repeat. The signature path reads the RAW ROW, not the normalized result, so one helper serves both — a fallback in the normalizer alone would fix the record and leave the loop collapsing |
| 4c | **#1468** | a `frontend_build` that RAN and FAILED carries its `exit_code`, `error` and bounded stderr; both copies of the row builder are single-sourced. **Found BY deploy A's own checkpoint pair**, which is the pair working: the React shakeout was rejected with `reason: ""` on an app the boot audit then built and booted, and the empty reason made every build failure render the identical signature element `frontend_build||failed` — an element that can never show progress, which pinned round-over-round movement at REPEAT and terminated the run `plan_defect` at round 1. The #878/#761 collapse on the check those fixes did not reach | **deploy A is superseded by it** (the shakeout-loop exit rule: a pair that produces a fix does not exit). Read on deploy B's pair: a failing build states its cause, and two different build failures no longer collapse to one element. The remaining per-module split is #1469, blocked on a corpus this fix produces |
