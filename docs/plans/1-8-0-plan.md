...
| 7 | **#598** | a SIP-0105 post-acceptance amendment first (the Dockerfile and nginx config are a **rendering** of the stack declaration, SIP-0102 §4.2's "environment definition is the contract"), then the rendering, with `container_packaging` re-pointed as the guard on the rendered files and the builder's role contract (SIP-0071) amended to stop authoring them | **a live prediction** (§4), restated as built: no counted roll's builder emits a container recipe, so `typed_checks.container_packaging_rows` reads unaskable (never evaluated) rather than zero findings; the frozen rendered `Dockerfile` is in every accepted tree; the builder's required `assembly_notes.md` is present (owner ruling 2026-09-13, SIP-0071 §12); the boot audit unchanged. The findings gate the rendering in CI instead |
...
termination reasons, with the carried, cleared and added counts #1501 puts on every `plan_defect` line; `dl` styling in the boot audit's screenshot;
...
- **Amendment (2026-09-17, with the 1.8.1 plan)** — everything this plan named as 1.8.1's is
  placed there by name in `docs/plans/1-8-1-plan.md`: the default flip (§3.3 step 7, §8 decision
  15) behind a fresh N on that line's deploy A, because §10c.4 read N = 5 of 6; the comparison
  window (§3.4 (d), §4.2) with the Solo arm of SIP-0108 §10h–§10j; the ops rider of §3.7, which
  this line's cut skipped; #1448 and #1039 from §3.6, not landed here. Superseded at the cut.
...
- **Rev 11 (2026-09-17)** — the deploy-F set closed: diagnostics 8/8, React 6/6, Next.js 2/3 (#1602), L1 held; §3c count N = 5 of 6 with qa × Next.js empty (its diagnostic supply was never registered — a pre-registration defect named in §10c.4); **the owner's ruling: the line closes on F**, the gate read as not met with the gap named (the 1.7.5 precedent); #1602, the Next.js own-frame diagnostic and #1600 are 1.8.1's
