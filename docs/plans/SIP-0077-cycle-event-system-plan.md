...
from squadops.api.runtime.deps import set_event_bus
...
Each route imports `get_event_bus` from deps. Event bus is obtained inside the try block (lazy import pattern matching existing code). If event bus is not configured, uses NoOpEventBus.

```python
# Example: cycle.created emission in cycles.py
from squadops.api.runtime.deps import get_event_bus
...
**deps.py `get_event_bus()` returns `NoOpEventBus` instead of raising RuntimeError** — this differs from other getters because event emission is best-effort, not required. Routes should never fail because the event bus is unconfigured.
...
| `src/squadops/api/runtime/deps.py` | `_event_bus`, `set_event_bus()`, `get_event_bus()` |
...
- **`get_event_bus()` returns `NoOpEventBus` not `RuntimeError`** — unlike other port getters. Event emission is best-effort; routes must never fail because the bus is unconfigured.
