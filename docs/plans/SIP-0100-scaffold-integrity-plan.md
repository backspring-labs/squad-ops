...
| # | Task | Files | Acceptance |
|---|------|-------|-----------|
| 1.1 | **Bind the harness criterion** to QA test slots. **NEXT.** Seam identified: `task_plan.py:544` (bind-mode `acceptance` assembly) — inject a `harness_boundary` TypedCheck per qa.test `expected_artifact` inside the QA namespace (`is_qa_test_path`). Needs a per-stack `harness_entry_modules(stack)` (add to `scaffold.py`, like `qa_test_namespace`) + the stack reachable there (from `contract.skeleton.expander`). | `scaffold.py`, `cycles/task_plan.py` | a bound qa.test task carries `harness_boundary` for each in-namespace test file. |
