...
| 0.5 | **Reason-code + atomicity taxonomy.** Define the contract-compliance family + distinct codes: `frozen_path_emission`, `unauthorized_slot_emission`, `undeclared_path_emission`, `post_write_integrity_fault`; fix response-atomic semantics (D5). | design note in this plan + `cycles/` evidence models | codes enumerated; each maps to a distinct corrective action (Review #15). |
...
| # | Task | Files | Acceptance |
|---|------|-------|-----------|
| 1.1 | **Bind the harness criterion** to the QA-owned test namespace (0.2): every QA test slot carries the scaffold-owned harness rule. | `scaffold_contract.py` | plan-binding check: a bound QA slot includes the harness criterion. |
| 1.2 | **Generated-artifact boundary check (post-generation).** After QA artifacts exist and before test execution, a **syntax-aware** (not substring) check fails a QA artifact that imports a stack-declared app entry module (`app.main`, `backend.main`, `import backend.main`, aliased, `importlib.import_module`) **or** directly constructs the stack's app test client. The stack owns the prohibited-entry list + approved fixture boundary. Indirect/dynamically-obscured bypasses are explicitly out of first scope. | `cycles/acceptance_checks.py` + `acceptance_check_spec.py` | direct app import → fail; direct `TestClient(app)` → fail; **fixture use → pass; a pure unit test with no app access → pass** (Review #6/#7). Runs at `patch_verification`, not plan validation (Review #5). |
| 1.3 | **QA proposer prompt (guidance only).** Fill-only appendix directs suites to use `client`; managed asset, not Python (#448). | `prompts/request_templates/request.qa_*`, `handlers/planning_tasks.py` | render test: appendix names `client`, forbids app imports. Prompt is guidance; 1.2 is the guarantee. |
| 1.4 | **Runner-shape harness proof.** Materialize the scaffold and run the fixture smoke test **through the production test-runner handler** — same handler, CWD, env, collection config a bound cycle uses (not a dev-shell `pytest`). | `tests/…` invoking `run_generated_tests` | fixture smoke reaches assertion under the runner's invocation. (Review #19.) |
