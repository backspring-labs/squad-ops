...
| # | Item | Effort | Notes |
|---|------|:------:|-------|
| **#233** | SIP-0089 §3.5 — route cycle recruitment through the coordinator to acquire a FocusLease. | **L** | Deferred-by-design (lease gate already enforced at coordinator; this adds a redundant-for-v1.1 executor path w/ stranded-lease risk on the critical path). Reason codes already minted+unused (`reasons.py:43-47`). **#244 depends on this.** |
| **#244** | SIP-0089 §4.5/D25 — wrap coordinator lease+activity+mode in one Postgres txn. | **L** | Needs a new `RuntimeTransaction` unit-of-work port (none exists). Path is unexercised in v1.1 → premature infra. **Land with/after #233** (the path that exercises the rollback). |
