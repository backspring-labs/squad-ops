...
| B1 #663 | structural | dropped context/seam | replay equivalence + both-seam tests | 19 goldens (11 dispatch + 5 correction + 3 plan-time) byte-identical through all three slices; both gate seams pinned (defs, call sites, returns-vs-raises) by `test_plan_gate_seams.py` | golden files + `test_context_assembly*_golden.py` | **green (PRs #751–#753)** |
| B2 #331 | structural | task identity drift | handler equivalence replay; stable task names | pure-move split AST-verified (20/20 top-level names, none lost/gained); all pre-split tests pass unmodified through the shim | the shim path itself (12 test files exercise it) | **green (PR #754)** |
| B3 #481 | recovery behavior | race / non-idempotence | seeded-state test + idempotent rerun + race case | read-only sweep = idempotent by construction (race costs one stale warning, D5 on issue); **first live boot 2026-08-07 surfaced 2 genuinely stranded cycles with exact recovery commands**, held 17 gate-rejections + 2 pending gates as visibility-only | boot log (deploy `df29d45c`) | **green (PR #755 + live boot)** |
| B4 #734 | additive provenance (spike disposition 2026-08-05) | id computed from a tree the evaluator never saw | content-addressing tests at both compute sites + goldens + replay zero-diff on stored corpus | **replay zero-diff 2026-08-07**: all 9 registry-replayable stored evaluation rows of `run_22af329fde29` re-evaluate identically post-change (5 non-replayable rows excluded by the registry's own `replayable` attribute) | replay-diff run over the shk-6 corpus | **green (PR #756 + replay)** |
| B4 #730 (+#504 D1) | registry extension + enforcement expansion (D1 ruling 2026-08-05) | new blocking check false-fails compliant slots | divergence-class tests + compliant-pass + injection guards; generated menu pinned by drift test | **surveillance discharged 2026-08-07**: shk-7 was the first natural bind-mode cycle — `fill_slot_signature` injected on the one claimed `.py` slot (`backend/routes.py`) and passed, silent on a compliant slot; no false-fail on the 3 `.jsx` slots (correctly out of scope per D3) | plan golden (5-route declared surface) + `typed_check_evaluation_task_0.json` | **green (PRs #757–#758 + shk-7)** |
...
| Gate-3 deploy window | line verification | refactor stack unproven on metal | deploy at Gate-3 head; loaded-module verification; owed live proofs banked | deploy `df29d45c` 2026-08-07: new modules verified loaded in runtime-api + agent images; #481 sweep live (above); #734 replay zero-diff (above); migrations no-op as expected | boot log + replay-diff output | **green — cut gate open** |
...
| Cut shakedown (shk-7) | line verification | the Gate-3 refactor stack breaks a live cycle | green unscored cycle on the fully integrated line | **GREEN 2026-08-06/07**: `cyc_b07183b3cf5c` on deploy `df29d45c` — verdict `accepted`, 36 checks executed / 36 passed, 15/15 contract criteria executed-and-passed, 0 corrections dispatched, 0 unreleased leases, 0 machinery defects across a 2-workload run (framing 21:17→22:12 ET, implementation 22:13→23:07 ET). Gate-3 stack exercised end-to-end: #663 registry served all 8 dispatches, #331 planning package authored the multi-role framing, `fill_slot_signature` (#730/#504) passed silent-on-clean, all 6 `typed_check_evaluation` artifacts carry a non-null `workspace_revision_id` (#734). One intra-task self-eval red (`frontend_compiles` on subtask 3, token-capped generation) resolved inside the designed self-eval loop — no cycle-level correction | the cycle's 6 evaluation artifacts + run report | **GREEN — cut authorized** |
