...
| B4 #730 (+#504 D1) | registry extension + enforcement expansion (D1 ruling 2026-08-05) | new blocking check false-fails compliant slots | divergence-class tests + compliant-pass + injection guards; generated menu pinned by drift test | registry extension is read-by-nothing enablement; `fill_slot_signature` is deterministic and unit-proven — first natural bind-mode cycle is the live check (surveillance, the AC#6 pattern) | plan golden (5-route declared surface) | **green (PRs #757–#758; surveillance)** |
