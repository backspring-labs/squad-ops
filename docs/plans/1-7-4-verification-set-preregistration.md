...
| **L2, L4, L7, L8** (1.7.3) | seam invariants | unlikely | the three 1.7.3 diagnostics re-run on A and on the pinned deploy | L2 reached twice on A inside the analyzer chain (`cyc_1063c4dca548`); the three re-runs in progress on A |
...
(done), absent-suite, own-frame-then-prose-repair, path-prefix (running back to back). All are re-run on the
