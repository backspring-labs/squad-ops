...
| **L2, L4, L7, L8** (1.7.3) | seam invariants | unlikely | the three 1.7.3 diagnostics re-run on A and on the pinned deploy | **L2, L4, L5, L7 reached on A** — absent-suite `cyc_1b54b29e2a99` (L2, two rounds) and the chain `cyc_a26c6828482c` (L7 → L4 → L5); L2 also twice inside the analyzer chain; L8 (path-prefix) running |
...
(done), absent-suite (done), own-frame-then-prose-repair (done), path-prefix (running). All are re-run on the
