...
def _routes_source(manifest: InterfaceManifest) -> str:
    ...
    # Function-local as in the Next.js module: scaffold.py imports this module.
    from squadops.capabilities.scaffold import root_persisted_entities

    store_names = [f"{_snake(name)}_store" for name in root_persisted_entities(manifest)]
...
def _store_source(manifest: InterfaceManifest) -> str:
    ...
    scaffold ownership. One dict per **root-persisted** entity, keyed by the entity's id
    (#1087): the manifest declares an entity for every shape it needs, and a table for a
    shape that is embedded in another row (``Participant`` inside ``Run.participants``) or
    projected from one (``RunSummary``, ``RunDetail``) told the developer to keep it in
    sync by hand and the qa author to assert on it — 1.6.6 roll 4's fill maintained three
    stores for one stored thing. The shapes are named here, with why they have no table,
    so neither author has to guess (the same rule drives the Next.js ``TABLES``).
    """
    from squadops.capabilities.scaffold import root_persisted_entities, shape_entities

    entities = list(root_persisted_entities(manifest))
    shapes = list(shape_entities(manifest))
    lines = [
        '"""In-memory state — scaffold-owned (frozen).',
        "",
        "The manifest declares in-memory persistence, so this module owns it: one store",
        "per root-persisted entity, keyed by the entity's id. Fill route bodies against",
        "these names; do not define a second store elsewhere and do not edit this file.",
    ]
    if shapes:
        lines.extend(
            [
                "",
                "Not stored on their own — shapes embedded in or projected from the rows",
                f"above, never rows themselves: {', '.join(shapes)}. Build them from a",
                "stored entity when a response needs them; do not keep a parallel store.",
            ]
        )
    lines.extend(
        [
...
def store_brief_lines(manifest: InterfaceManifest) -> list[str]:
    ...
    stores = ", ".join(f"`{_snake(name)}_store`" for name in root_persisted_entities(manifest))
    shapes = ", ".join(f"`{name}`" for name in shape_entities(manifest))
    no_table = (
        f" {shapes} have no store — they are shapes embedded in or projected from those "
        "rows; build them from a stored entity, never keep a parallel store."
        if shapes
        else ""
    )
    return [
