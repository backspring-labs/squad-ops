...
one language (``structural_python`` for ``.py``, ``structural_jsx`` for JavaScript, JSX, TypeScript and TSX); this
...
def reads_structure(path: str) -> bool:
    return path.endswith(PYTHON_SUFFIXES) or path.endswith(ECMASCRIPT_SUFFIXES)
...
def resolve_entity(path: str, content: str, selector: str) -> list[tuple[int, int]] | None:
    ...
    if path.endswith(ECMASCRIPT_SUFFIXES):
...
def entity_selectors(path: str, content: str) -> tuple[str, ...] | None:
    ...
    elif path.endswith(ECMASCRIPT_SUFFIXES):
