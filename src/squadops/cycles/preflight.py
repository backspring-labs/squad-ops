...
def stack_development_profile_decision(config: Mapping[str, Any]) -> PreflightDecision:
    ...
    """
    from squadops.capabilities.scaffold import development_profile_for, resolve_development_profile

    if resolve_development_profile(config) is not None:
        return PreflightDecision()

    build_profile = str(config.get("build_profile") or "")
    required = development_profile_for(build_profile)
    declared = str(config.get("development_profile") or "")
    return PreflightDecision(
        blocking=(
            Finding(
                code="stack_development_profile_mismatch",
                severity="block",
                message=(
                    f"this cycle declares its stack twice and the two disagree: "
                    f"`build_profile` is `{build_profile}`, whose scaffold requires "
                    f"development profile `{required}`, but `development_profile` is `{declared}`. "
                    ...
                    f"`development_profile: {required}` in the request profile or "
