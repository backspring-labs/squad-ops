...
class SquadOpsConfig:
    ...
    def get_tasks_backend(self) -> str:
        ...
        return os.getenv("TASKS_BACKEND", "prefect").lower()  # Default changed to prefect

    def get_prefect_api_url(self) -> str:
        """Get Prefect API URL"""
        return os.getenv("PREFECT_API_URL", "http://prefect-server:4200/api")

    def get_prefect_api_key(self) -> str | None:
        """Get Prefect API key (optional, for authentication)"""
        return os.getenv("PREFECT_API_KEY", None)
    ...
    def get_llm_config(self, key: str | None = None) -> Any:
        ...
        """
        llm_config = {
            "url": self._get_env_with_platform_fallback(
